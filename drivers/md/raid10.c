			     ...
			     int *skipped)
{
 ...
 skipped:
	...
	if (mddev_is_clustered(mddev) &&
		...
		/*
		 * For recovery, the curr_resync_completed represents the
		 * address of a single device. So we need to transform it
		 * to array address first, then inform other nodes with
		 * the array address.
		 */
		if (test_bit(MD_RECOVERY_RECOVER, &mddev->recovery)) {
			conf->cluster_sync_low = 0;
			for (i = 0; i < conf->geo.raid_disks; i++) {
				sector_t sect = raid10_find_virt(conf,
					mddev->curr_resync_completed, i);
				if (conf->cluster_sync_low == 0)
					conf->cluster_sync_low = sect;
				if (conf->cluster_sync_low > sect &&
				    conf->cluster_sync_low != 0)
					conf->cluster_sync_low = sect;
			}
		} else
